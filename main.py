...
class SheetAnimImage(Widget):
    ...
    def _redraw(self, *_):
        ...
        with self.canvas:
            GColor(1, 1, 1, 1)
            if self.scale_x < 0:
                # Flip orizzontale usando le UV reali della sub-texture.
                # tex_coords = (u_BL,v_BL, u_BR,v_BR, u_TR,v_TR, u_TL,v_TL)
                # Per flip X: scambia u di BL<->BR e u di TL<->TR, v invariato.
                tc = tex.tex_coords
                flipped = (tc[2],tc[1], tc[0],tc[3], tc[6],tc[5], tc[4],tc[7])
